// test.js - Decide Search 功能测试页面脚本
...
/**
 * 加载测试用例到输入框
 * @param {string} type - 测试用例类型
 */
function loadTestCase(type) {
  const testInput = document.getElementById('test-input');
  testInput.value = testCases[type] || '';
}
...
async function testSplit() {
  ...
  try {
    const currentText = document.getElementById('test-input').value.trim();
    const results = await splitText(currentText, mode, options);
...
function testDetectType() {
  ...
  try {
    const currentText = document.getElementById('test-input').value.trim();
    const detection = detectContentType(currentText);
...
function testExtractInfo() {
  ...
  try {
    const currentText = document.getElementById('test-input').value.trim();
    const formats = analyzeTextForMultipleFormats(currentText);
...
function displaySplitResults(results, mode) {
  ...
  for (let i = 0; i < results.length; i++) {
    html +=
      '<span class="split-item" title="第' + (i + 1) + '项">' + escapeHtml(results[i]) + '</span>';
  }
  html += '</div>';
...
function displayTypeResults(detection) {
  ...
  let featuresHtml = '';
  const entries = Object.entries(detection.features);
  for (let i = 0; i < entries.length; i++) {
    featuresHtml += '<li>' + entries[i][0] + ': ' + entries[i][1] + '</li>';
  }
...
function displayExtractResults(formats) {
  ...
  for (let i = 0; i < formats.length; i++) {
    ...
    if (item.data && item.data.length > 0) {
      html += '<div><strong>' + item.type + ':</strong><ul>';
      for (let j = 0; j < item.data.length; j++) {
        html += '<li>' + escapeHtml(item.data[j]) + '</li>';
      }
      html += '</ul></div>';
    }
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('btn-chinese').addEventListener('click', function () {
    loadTestCase('chinese');
  });
  document.getElementById('btn-english').addEventListener('click', function () {
    loadTestCase('english');
  });
  document.getElementById('btn-mixed').addEventListener('click', function () {
    loadTestCase('mixed');
  });
  ...
  document.getElementById('btn-wrapped').addEventListener('click', function () {
    loadTestCase('wrapped');
  });
  document.getElementById('btn-complex').addEventListener('click', function () {
    loadTestCase('complex');
  });
