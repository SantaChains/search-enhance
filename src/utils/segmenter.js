...
export class Segmenter {
    ...
    async segment(text, options = {}) {
        ...
        // 对于大型文本，使用Web Worker
        if (useWorker && this.worker && text.length > 500) {
            logger.debug('使用Web Worker进行分词');
            try {
                results = await Promise.race([
                    this.workerSegment(text, options),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Worker分词超时')), timeout))
                ]);
            } catch (error) {
                logger.warn('Worker分词失败，回退到主线程:', error.message);
        ...
        logger.debug(`分词完成，处理时间: ${processingTime.toFixed(2)}ms`);
    ...
    postProcess(results) {
        ...
        // 5. 移除包含特殊字符的无意义分词
        const cleanedResults = sortedResults.filter(item => {
            // 移除纯数字（可能是随机ID或时间戳）
            if (/^\d+$/.test(item)) return false;
            // 移除纯符号
            if (/^[^a-zA-Z0-9\u4e00-\u9fa5]+$/.test(item)) return false;
            return true;
        });
...
class ChineseSegmenterPlugin {
    ...
    async segment(text, options = {}) {
        ...
        // 智能合并相邻的短分词（如果它们语义上可能相关）
        const mergedResults = this.mergeShortSegments(results);
