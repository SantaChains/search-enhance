...
passed += test('intelligentSegmentation - 长文本', () => {
    const longText = '这是一个很长的测试文本，用于测试智能分词功能的性能。中文分词是自然语言处理的基础技术。';
...
passed += test('processMultiFormat - 混合内容', () => {
    const result = processMultiFormat('Contact test@example.com, visit https://a.com, server 192.168.1.1');
