...
const logger = {
  ...
  error: (msg, ...args) =>
    console.error(`[SearchBuddy-Global] ${msg}`, ...args),
...
async function pollClipboard() {
  ...
  try {
    ...
    const isNewContent =
      text !== storedContent && text !== appState.lastContent;
...
async function startPolling() {
  ...
  appState.pollTimer = setInterval(
    pollClipboard,
    CLIPBOARD_CONFIG.POLL_INTERVAL,
  );
...
function removeNotification() {
  const notification = document.getElementById(
    "search-buddy-global-notification",
  );
  if (notification) {
      ...
      switch (request.action) {
        case "toggleGlobalMonitoring":
          ...
          if (
            ...
            request.isActive !== appState.isMonitoring
          ) {
...
async function initialize() {
  ...
  try {
    ...
    logger.info(
      "从storage读取 globalMonitoringEnabled:",
      result.globalMonitoringEnabled,
    );
    ...
    // 加载最近的剪贴板内容
    const contentResult = await chrome.storage.local.get(
      CLIPBOARD_CONFIG.STORAGE_KEY,
    );
