...
async function loadState() {
  try {
    ...
    appState.isMonitoring =
      result[CONFIG.STORAGE_KEYS.GLOBAL_MONITORING] ??
      CONFIG.DEFAULT_MONITORING;
    appState.lastContent = result[CONFIG.STORAGE_KEYS.CLIPBOARD_CONTENT] || "";

    logger.info(
      `加载状态: 监控=${appState.isMonitoring}, 内容长度=${appState.lastContent.length}`,
    );
  } catch (error) {
...
async function handleClipboardChange(content, version, source = "background") {
  if (
    ...
    version === appState.lastBroadcastVersion
  ) {
  ...
  logger.info(
    `剪贴板变化已广播 (来源: ${source}): ${content.substring(0, 50)}...`,
  );
  return { success: true };
}
...
chrome.runtime.onConnect.addListener((port) => {
  if (
    port.name.startsWith("global-clipboard-") ||
    ...
    logger.info(
      `Port连接: ${port.name}, 当前连接数: ${appState.activeTabs.size}`,
    );
    ...
    port.onDisconnect.addListener(() => {
      appState.activeTabs.delete(tabId);
      logger.info(
        `Port断开: ${port.name}, 当前连接数: ${appState.activeTabs.size}`,
      );
    });
  }
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  (async () => {
    switch (request.action) {
      ...
      case "clipboardChanged":
        const result = await handleClipboardChange(
          request.content,
          request.version,
          "message",
        );
