...
chrome.commands.onCommand.addListener(async (command, tab) => {
    ...
    switch (command) {
        ...
        case 'toggle_clipboard_monitoring':
            ...
            try {
                globalClipboardState.isMonitoring = !globalClipboardState.isMonitoring;
                
                if (globalClipboardState.isMonitoring) {
                    globalClipboardState.monitoringTabs.add(tab.id);
                    await chrome.tabs.sendMessage(tab.id, { 
                        action: 'toggleClipboardMonitoring' 
                    });
                    logger.info('剪贴板监控已开启');
                } else {
                    globalClipboardState.monitoringTabs.delete(tab.id);
                    await chrome.tabs.sendMessage(tab.id, { 
                        action: 'toggleClipboardMonitoring' 
                    });
                    logger.info('剪贴板监控已关闭');
                }
                
                // 发送状态通知
                await chrome.tabs.sendMessage(tab.id, {
                    action: 'showNotification',
                    message: globalClipboardState.isMonitoring ? '剪贴板监控已开启' : '剪贴板监控已关闭',
                    type: globalClipboardState.isMonitoring ? 'success' : 'info'
                });
