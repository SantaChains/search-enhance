...
class ClipboardMonitor {
    ...
    async adjustPollingInterval(hasChange) {
        if (hasChange) {
            ...
            this.pollingConfig.currentInterval = Math.max(
                ...
                this.pollingConfig.currentInterval - 50 // 更小的步长，更平滑的调整
            );
            this.pollingConfig.noChangeCount = 0;
            this.pollingConfig.totalNoChangeCount = 0; // 重置总无变化计数
        } else {
            // 无变化：增加无变化计数
            this.pollingConfig.noChangeCount++;
            this.pollingConfig.totalNoChangeCount++;
            
            // 当连续无变化达到阈值时，增加轮询间隔，节省资源
    ...
    /**
     * 检查剪贴板权限
     * @returns {boolean} 是否有权限读取剪贴板
     */
    async checkClipboardPermission() {
        // 检查权限缓存，避免频繁调用权限API
        if (this.permissionCache && Date.now() - this.permissionCache.timestamp < 300000) { // 5分钟缓存
            return this.permissionCache.hasPermission;
        }
        
        try {
            // 使用新的剪贴板工具检查权限
            const hasPermission = await checkClipboardPermissionUtil();
