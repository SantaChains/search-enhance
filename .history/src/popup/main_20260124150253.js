        ...
        return `
            <div class="history-item" data-id="${item.id}">
                <div class="clipboard-item-content">
                    ${appState.clipboardBatchMode ? `<input type="checkbox" class="clipboard-checkbox" data-id="${item.id}">` : ''}
                    <div class="clipboard-text">
                        <pre class="clipboard-text-content">${escapeHtml(item.text)}</pre>
                        <div class="clipboard-actions">
                            <button class="copy-btn btn-sm" data-id="${item.id}">复制</button>
                            <button class="edit-btn btn-sm" data-id="${item.id}">编辑</button>
                            <button class="delete-btn btn-sm" data-id="${item.id}">删除</button>
                        </div>
                    </div>
                </div>
            </div>
...
// 编辑剪贴板项
function editClipboardItem(id) {
    const item = appState.clipboardHistory.find(item => item.id === id);
    if (!item) return;
    
    const newText = prompt('编辑剪贴板内容:', item.text);
...
function toggleBatchOperations() {
    ...
    // 显示/隐藏批量操作工具栏
    elements.batch_toolbar.style.display = appState.clipboardBatchMode ? 'flex' : 'none';
    
    // 更新批量操作按钮文本
    elements.batch_operations_btn.textContent = appState.clipboardBatchMode ? '取消批量' : '批量操作';
